...
class Leaderboard extends React.Component {
  ...
  componentWillMount(this.props.dispatch(addScoreToUserObj(this.props.users)));
  
  usersLeaderboard = () => {
     usersArr = Object.keys(this.props.users);
     return usersArr.sort(compareFunction(a,b){return(a.score - b.score)});
  }
              ...
              {this.usersLeaderboard.map( user => {
                return (
                  <li className="card" key=`${user.id}`>
                    <div className="user">
                      <img src=`${user.avatarURL}` alt="userAvatar"></img>
                      <div className="userName">${user.name}</div>
                    </div>
                    <div className="scoreContainer">
                      <p className="item"> Answered questions: <span>${Object.keys(user.answers).length}</span></p>
                      <p className="item"> Created questions: <span>${user.questions.length}</span></p>
